...
class User():
    ...
    def password(self, pwd):
        ...
        - Hash `pwd` in MD5 before assigning to `__password`  # Fixed this comment
        """
        if pwd is None or type(pwd) is not str:
            self.__password = None
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower()  # Fixed the attribute name
    ...
    def is_valid_password(self, pwd):
        ...
        if self.__password is None:
            return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # Use lower() for comparison
